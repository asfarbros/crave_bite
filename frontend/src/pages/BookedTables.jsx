...
function BookedTables() {
  ...
  const loadBookings = async () => {
    ...
    if (!token) {
      setError("Please login to view your table bookings.");
      setLoading(false);
      return;
    }

    try {
      const res = await fetch(`${API_URL}/api/booking/mybookings`, {
    ...
    <div className="bg-gradient-to-b from-[#fefae0] to-[#fdf6e3] min-h-screen text-gray-800 flex-grow py-12 px-4 sm:px-6">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-10">
          <h1 className="text-4xl font-bold text-yellow-700 mb-2">Booked Tables</h1>
          <p className="text-gray-500">Your table reservations, all in one place.</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center bg-white rounded-2xl shadow-md py-16 px-6">
            ...
            <p className="text-xl text-gray-500 mb-6">No table bookings yet.</p>
            <Link to="/booking" className="inline-block bg-yellow-500 hover:bg-yellow-600 text-black font-bold py-3 px-8 rounded-full transition duration-300 shadow-md hover:scale-105">
              Book a Table
            </Link>
          </div>
        ) : (
                  ...
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-2">
                    <div>
                      ...
                      <p className="text-lg font-bold text-yellow-800">
                        {booking.tableIcon} {booking.tableName}
